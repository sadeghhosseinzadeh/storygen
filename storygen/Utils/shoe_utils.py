...
def place_shoe_1c(
    ...
    shadow_blur=35,
    # --- shadow tuning ---
    toe_width=90,          # thickness at front
    heel_width=20,         # thickness at end
    shadow_length=0.55,    # % of shoe length that gets shadow
    shadow_alpha=150,      # base darkness
    shadow_height=30       # vertical thickness of shadow band
):
    """
    Shoe placement for template 1c:
    - Centers AFTER rotation using toe→heel midpoint
    - Shadow is a tunable flat ground shadow under the sole
    """
    ...
    if shadow:
        ...
        # Compute shadow end (shorter than full shoe)
        shadow_end_x = int(toe_x + (heel_x - toe_x) * shadow_length)
        ...
        # Mid band (between toe and shadow_end_x)
        shadow_draw.rectangle(
            ...
            fill=(0, 0, 0, max(0, shadow_alpha - 40)),
        )
        ...
        shadow_draw.ellipse(
            ...
            fill=(0, 0, 0, max(0, shadow_alpha - 80)),
        )
